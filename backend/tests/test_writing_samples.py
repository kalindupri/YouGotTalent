...
def test_draft_sample_not_visible_to_public(client, talent_headers, talent_profile):
...
def test_owner_sees_own_draft_via_public_profile_when_authenticated(client, talent_headers, talent_profile):
    add_sample(client, talent_headers, is_published=False)

    profile_resp = client.get(f"/api/v1/talents/{talent_profile['id']}", headers=talent_headers)
    samples = profile_resp.json()["writing_samples"]
    assert len(samples) == 1
    assert samples[0]["is_excerpt"] is False


def test_published_sample_truncated_to_visible_lines_for_non_owner(client, db_session, talent_headers, talent_profile):
    ...
    resp = client.post(
        "/api/v1/talents/me",
        json={"display_name": "Reader One", "category": "acting", "city": "Kandy"},
        headers=other_headers,
    )
    assert resp.status_code == 201, resp.text

    profile_resp = client.get(f"/api/v1/talents/{talent_profile['id']}", headers=other_headers)
    samples = profile_resp.json()["writing_samples"]
    assert len(samples) == 1
...
def test_guest_gets_truncated_body_too(client, talent_headers, talent_profile):
    body = "\n".join(f"Line {i}" for i in range(1, 21))
...
def test_short_piece_under_visible_lines_is_not_marked_excerpt(client, talent_headers, talent_profile):
...
def test_invalid_writing_type_rejected(client, talent_headers, talent_profile):
    resp = add_sample(client, talent_headers, writing_type="screenplay-of-doom")
    assert resp.status_code == 400


def test_invalid_language_rejected(client, talent_headers, talent_profile):
    resp = add_sample(client, talent_headers, language="klingon")
    assert resp.status_code == 400


def test_all_supported_languages_accepted(client, talent_headers, talent_profile):
...
def test_all_supported_writing_types_accepted(client, talent_headers, talent_profile):
...
def test_free_tier_publish_limit_enforced(client, talent_headers, talent_profile):
    for i in range(3):
...
def test_free_tier_drafts_dont_count_toward_publish_limit(client, talent_headers, talent_profile):
    for i in range(5):
...
def test_premium_talent_not_limited(client, talent_headers, talent_profile):
...
def test_talent_edits_own_draft_then_publishes(client, talent_headers, talent_profile):
...
def test_talent_lists_own_samples_including_drafts(client, talent_headers, talent_profile):
...
def test_talent_deletes_own_sample(client, talent_headers, talent_profile):
...
def test_talent_cannot_edit_or_delete_another_talents_sample(client, db_session, talent_headers, talent_profile):
    sample = add_sample(client, talent_headers, is_published=True).json()
    ...
    resp = client.post(
        "/api/v1/talents/me",
        json={"display_name": "Other Writer", "category": "script_writing", "city": "Galle"},
        headers=other_headers,
    )
    assert resp.status_code == 201, resp.text
